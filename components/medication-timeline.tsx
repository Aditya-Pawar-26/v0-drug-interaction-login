      ...
      <div className="relative space-y-6 pl-8">
        ...
        {medications.map((med, index) => (
          <div key={med.id} className="relative">
            {/* Timeline dot */}
            <div className="absolute -left-7 top-2 w-5 h-5 rounded-full bg-white border-4 border-primary ring-2 ring-primary/10" />
            ...
            <Card className="bg-card border border-border/40 hover:border-primary/50 hover:shadow-md transition-all p-5 space-y-4">
              <div className="flex items-start justify-between gap-4">
                ...
                <Badge className={`flex items-center gap-1 h-fit whitespace-nowrap border ${getSeverityColor(med.severity)}`}>
                  <span>{getSeverityIcon(med.severity)}</span>
